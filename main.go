...
func main() {
	...
	var openCmd = &cobra.Command{
		Use:   "open <mins>",
		Short: "Unlock the vault for a set time (creates session)",
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err := src.CreateSession(masterPassword, duration, readonly); err != nil {
			...
			fmt.Printf("Open mode activated for %d minutes (%s).\n", mins, mode)
		},
	}
	openCmd.Flags().Bool("readonly", false, "Open in read-only mode")
	...
	unlockCmd.Flags().Bool("readonly", false, "Open in read-only mode")
	...
	rootCmd.AddCommand(initCmd, addCmd, getCmd, delCmd, editCmd, genCmd, modeCmd, cinfoCmd, scanCmd, auditCmd, openCmd, unlockCmd, lockCmd, totpCmd, importCmd, exportCmd)
	rootCmd.Execute()
}
