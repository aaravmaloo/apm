...
const ENROLL_MIN_CLEAN: usize = 15;
const ENROLL_MAX_SPREAD: f32 = 0.40; // Even more variance for diverse angles
const VERIFY_MAX_SPREAD: f32 = 0.20; // Stricter template merging = more unique templates
const VERIFY_MIN_CLEAN: usize = 3;
const VERIFY_THRESHOLD_BUMP: f32 = 0.12; // High tolerance for stable faces
...
pub unsafe extern "C" fn faceid_enroll(
    ...
    catch_status(err_buf, err_buf_len, || {
        ...
        let target_frames = if num_frames < 40 {
            80 // ~4-5 seconds of diverse angle capture
        } else {
            num_frames as usize
        };
        ...
        // Instruct user (via log or wait) to move slightly
    ...
    catch_status(err_buf, err_buf_len, || {
        ...
        for _ in 0..max_frames {
            ...
            if stable.len() >= VERIFY_MIN_CLEAN {
                ...
                // If the face is stable, we allow a much more relaxed threshold
                // to account for laptop angles/weird positions.
