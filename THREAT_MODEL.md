...
*   **Plaintext Leakage**: Sensitive data is only decrypted into memory and wiped (`Wipe()`) as soon as the operation completes.
*   **Key Reuse Attacks**: Every vault initialization and every save uses fresh, high-entropy salts and nonces.
...
*   **Process Persistence**: While APM wipes its own buffers, it cannot guarantee that the OS won't swap pages containing sensitive data to disk (swap/hibernation).
